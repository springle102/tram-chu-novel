...
export default function NewChapterPage() {
  ...
  useEffect(() => {
    ...
    setRole(user.role);
    if (user.role !== 'author') {
      setMessage({ type: 'error', text: 'Chỉ có Tác giả mới được quyền đăng chương mới.' });
      setLoading(false);
      return;
    }
  ...
  const loadStoryDetails = async () => {
    try {
      const res = await fetchAdmin(`/api/admin/stories/${storyId}/chapters`);
      if (res.success && res.data) {
        setStoryTitle(res.data.story.title);
      } else {
        throw new Error(res.error || 'Không tải được thông tin truyện.');
      }
      ...
      setMessage({ type: 'error', text: err.message || 'Lỗi kết nối hệ thống.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setMessage({ type: 'success', text: 'Đăng chương mới thành công! Đang quay lại danh sách...' });
        setTimeout(() => {
          router.push(`/admin/stories/${storyId}`);
        }, 1500);
      } else {
        throw new Error(res.error || 'Đăng chương thất bại.');
      }
    } catch (err: any) {
      setMessage({ type: 'error', text: err.message || 'Lỗi kết nối khi đăng chương.' });
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="py-12 flex justify-center items-center">
        ...
        <span className="text-gray-500 font-semibold text-sm">Đang tải thông tin...</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-gray-400">
        ...
        <span>/</span>
        <span className="text-gray-600">Đăng chương mới</span>
      </div>

      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Đăng chương mới</h1>
        <p className="text-sm text-gray-500 mt-1">
          Đang thêm chương cho bộ truyện <strong className="text-purple-600 font-semibold">"{storyTitle}"</strong>.
        </p>
      </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
              ...
              <div className="space-y-2 pt-4 border-t border-gray-100">
                ...
                >
                  {submitting ? 'Đang lưu...' : publishMode === 'draft' ? 'Lưu bản nháp' : publishMode === 'schedule' ? 'Lưu & Đặt lịch' : 'Đăng chương'}
                </button>
