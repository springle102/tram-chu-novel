...
export default function AdminAuthorsPage() {
  ...
  // View stories states
  const [viewingAuthor, setViewingAuthor] = useState<{ id: string; username: string } | null>(null);
  ...
  useEffect(() => {
    if (viewingAuthor) {
      loadAuthorStories(viewingAuthor.id);
    } else {
      setAuthorStories([]);
    }
  }, [viewingAuthor]);
  ...
  const loadAuthors = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setTotalAuthors(res.data.pagination.total || 0);
      } else {
        throw new Error(res.error || 'Không tải được danh sách tác giả.');
      }
    } catch (err: any) {
  ...
  const loadAuthorStories = async (authorId: string) => {
    setStoriesLoading(true);
    try {
      const res = await fetchAdmin(`/api/admin/stories?authorId=${authorId}&limit=50`);
      if (res.success) {
        setAuthorStories(res.data.stories || []);
      } else {
        alert(res.error || 'Không thể lấy danh sách truyện của tác giả.');
      }
    } catch (err: any) {
      alert(err.message || 'Lỗi kết nối API lấy truyện.');
    } finally {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white border border-gray-150 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="py-12 flex justify-center items-center">
            ...
            <span className="text-gray-500 font-semibold text-sm">Đang tải danh sách tác giả...</span>
          </div>
        ) : error ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/50 text-xs text-gray-400 font-bold uppercase">
                    ...
                    <th className="py-4 px-6 text-center">Tác phẩm</th>
                  ...
                  {authors.length === 0 && (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-gray-400 font-semibold">
                        Không tìm thấy tác giả thỏa mãn.
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {/* Viewing Author Stories Modal */}
      {viewingAuthor && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-gray-150 animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {storiesLoading ? (
                <div className="py-20 flex justify-center items-center flex-col gap-3">
                  ...
                  <span className="text-xs text-gray-500 font-semibold">Đang truy vấn kho truyện của tác giả...</span>
                </div>
              ) : authorStories.length === 0 ? (
                <div className="py-20 text-center text-gray-450 font-semibold text-sm">
                  Tác giả này hiện chưa đăng tải bộ truyện nào trên hệ thống.
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-gray-200 text-xs text-gray-400 font-bold uppercase">
                        <th className="pb-3 w-[70px]">Bìa</th>
                        <th className="pb-3 px-4">Tên truyện</th>
                        <th className="pb-3 px-4">Thể loại</th>
                        <th className="pb-3 px-4 text-center">Trạng thái</th>
                        <th className="pb-3 px-4 text-right">Lượt xem</th>
                        <th className="pb-3 px-4 text-center">Đánh giá</th>
                        <th className="pb-3 px-4 text-center">Chương</th>
                      </tr>
                    </thead>
                    <tbody className="text-sm divide-y divide-gray-100">
                        ...
                        <tr key={story.id} className="hover:bg-gray-50/50">
                          <td className="py-3">
                            <img
                              ...
                              alt={story.title}
                              className="w-9 h-12 object-cover rounded-md border border-gray-150 shadow-sm"
                            />
                          </td>
                          <td className="py-3 px-4">
                            <div className="font-bold text-gray-800">{story.title}</div>
                            <span className="text-[10px] text-gray-400 font-mono mt-0.5">{story.slug}</span>
                          </td>
                          <td className="py-3 px-4 text-gray-550 font-medium">{story.category_name || 'Không có'}</td>
                          ...
                          <td className="py-3 px-4 text-center font-bold text-gray-700">{story.chapter_count}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {/* Modal Footer */}
            <div className="p-4 border-t border-gray-150 bg-gray-50 rounded-b-2xl flex justify-end">
              <button
                onClick={() => setViewingAuthor(null)}
                className="px-4 py-2 border border-gray-250 bg-white rounded-xl text-xs font-bold text-gray-650 hover:bg-gray-50 transition-all"
              >
                Đóng lại
              </button>
            </div>
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-150 animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-lg font-bold text-gray-800">
              {selectedUser.is_banned ? 'Mở khóa tài khoản?' : 'Khóa tài khoản tác giả?'}
            </h3>
            <p className="text-sm text-gray-500 mt-2 leading-relaxed">
              Bạn đang thực hiện {selectedUser.is_banned ? 'mở khóa' : 'khóa'} tài khoản tác giả tác phẩm{' '}
